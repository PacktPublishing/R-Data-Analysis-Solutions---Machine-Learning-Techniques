...
train.idx <- createDataPartition(bn$class, p = 0.7, 
                                 list =FALSE)

mod <- rpart(class ~ ., data = bn[train.idx, ],
            method ="class", 
...
table(bn[-train.idx,]$class, pred.pruned,
      dnn = c("Actual","Predicted"))


pred.pruned <- predict(mod, bn[-train.idx,],
                       type = "prob")
...
mod <- randomForest(x = bn[sub.idx,1:4], y=bn[sub.idx,5],
                    ntree=500, keep.forest=TRUE)

pred <- predict(mod, bn[-sub.idx,])

table(bn[-sub.idx,"class"], pred, 
      dnn = c("Actual","Predicted"))
...
table(bn[t.idx,"class"], fitted(mod),
      dnn = c("Actual","Predicted"))

pred <- predict(mod, bn[-t.idx,])

table(bn[-t.idx, "class"], pred,
      dnn = c("Actual", "Predicted"))
...
train.idx <- createDataPartition(ep$Purchase, p = 0.67,
                                 list = FALSE)
...
tab <- table(ep[-train.idx,]$Purchase, pred,
             dnn = c("Actual","Predicted"))
...
train.idx <- createDataPartition(vac$Result, p = 0.5,
                                 list =FALSE)

train <- vac[train.idx, ]
temp <- vac[-train.idx, ]
val.idx <- createDataPartition(temp$Result, p = 0.5,
                               list =FALSE)
val <- temp[val.idx, ]
test <- temp[-val.idx, ]
...
errmat.test = table(test$Result, pred.test,
                      dnn = c("Actual","Predicted"))
...
mod <- nnet(class ~., data=bn[train.idx,],size=3,maxit=10000,
            decay=.001, rang = 0.05)
...
table(bn[t.idx, "class"], bn[t.idx, "Pred"],
      dnn = c("Actual","Predicted"))
...
bh[-train.idx,"PROB_SUCC"] <- predict(logit, newdata = bh[-train.idx,],
                                      type="response")
